...
from app.favorites.exceptions import RefreshTokenExpireError, \
...
def create_token(data: dict, expires_delta: timedelta, token_type: str) -> str:
    ...
    token = jwt.encode(encode, settings.SECRET_KEY, algorithm=settings.ALGORITHM), token_id
...
def create_access_token(self, data: dict):
    return self.create_token(data, timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES), "access")

def create_refresh_token(self, data: dict):
    return self.create_token(data, timedelta(days=settings.REFRESH_EXPIRE_DAYS), "refresh")
